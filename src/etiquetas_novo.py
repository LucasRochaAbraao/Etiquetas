...
# Uma opção seria rodar sem alterar o DB por padrão e caso
# seja para produção passar um parâmetro. e.g.: -p 250
# - Execute the program with a GUI  (pyimgui)
...
LOG_PATH     = os.path.join(PROJECT_PATH, 'log')  # /etiquetas/log
EMAIL        = os.path.join(PROJECT_PATH, 'email') # /etiquetas/email
SRC_FILE_BGN = os.path.join(PROJECT_PATH, 'templates/template-bgn.xlsx') # etiquetas/templates/template-bgn.xlsx
SRC_FILE_AC  = os.path.join(PROJECT_PATH, 'templates/template-ac.xlsx')  # etiquetas/templates/template-ac.xlsx
END_FILE_BGN = 'etiquetas-bgn.xlsx'
END_FILE_AC  = 'etiquetas-ac.xlsx'
SRC_FILE_PATRIMONIO = os.path.join(PROJECT_PATH, 'templates/template-patrimonio.xlsx') # etiquetas/templates/template-patrimonio.xslx
END_FILE_PATRIMONIO = 'patrimonio.xslx'
YAML_FILE = os.path.join(PROJECT_PATH, 'src', 'historico_etiquetas.yml')
...
class Etiquetas():
    ...
    def __init__(self):
        ### CONFIG LOGGER ###
        LOG_FORMAT = "%(asctime)s [%(levelname)s]:\t%(message)s" # 2020-08-18 08:47:23,399 [INFO]: mensagem
